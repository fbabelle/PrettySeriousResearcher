...
4. **Legibility & validity.** Is it readable and correct? For research code specifically: does it preserve **experimental validity** — no data leakage, correct train/val/test temporal split, fixed/recorded seeds, metrics computed as claimed, results reproducible from the committed code? Also trace each hypothesis of every guarantee the paper states to the call site that runs in the reported cells; the paper must describe the executed construction, and a latent defect in an unexercised branch (a merge normalisation valid only under fixed membership) becomes an open amendment before that branch is ever used, not an implicit claim (earned 2026-09-16).
...
**Horizon consistency belongs on the environment-audit checklist:** list every place a time horizon or frequency enters (the label, the certificate's statistic, the threshold's cost model, the book's calendar, the family's declared profile) and check they agree per family — a disagreement is a defect that no amount of downstream analysis repairs (2026-09-12).
...
- **Re-audit the prompts whenever the scope or an experimental axis changes, and check information parity.** Read each role's prompt against what the runtime actually renders: every input the prompt *promises* must be *shown* (earned: a direction role was promised economic stories, regime covariates, wealth and notes and received none of them for the whole campaign). When an LLM arm is compared with an algorithmic baseline, state whether they saw the same information — an "LLM ≈ bandit" result under information parity is a different finding from one where the LLM had context the bandit lacked. New members of an axis (families, tasks) must arrive with the description the prompt promises, not as bare names. Prompt changes are amendments: never under a running block, always with a labelled A/B slice.
