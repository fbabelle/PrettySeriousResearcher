...
- **Screen on a slice when the candidates outnumber what can be built.** Implement each cheaply and run it on a small, representative slice of the benchmark (a development period or subset, never the test or holdout partition) against the *reproduced* baseline (reproduce it first, as Step 4 describes), then give one verdict per candidate under a rule written before the runs: **drop** (clearly worse), **scale up** (consistently better), or **engineer** (promising but needs tuning or a fix), with a cap on engineering rounds per candidate after which it is dropped. State every cap before the loop starts: candidates screened, engineering rounds each, refinement rounds (evolved ideas are mixed with the next untried reserve cards from `research-topic-selection`), and the number of survivors needed to stop. ScientistTwo's best ideas came mostly from the first refinement rounds (arXiv:2609.19644, Fig. 10), so 2–3 rounds is a sound default; zero survivors at the cap goes to the orchestrator's null-result branch, not to another round. Every candidate screened, engineered or refined is a **trial**: log it with its slice result so `research-finance-rigor` can count the whole family (ScientistTwo reports its gains without seed variance or significance tests, so its selection over many ideas on the benchmark it reports is exactly the winner's-curse risk this counting exists for).
...
- **Reproduce the named SOTA in this harness before measuring anything against it.** Run the baseline's published configuration through the same data, splits and evaluator the method will use, and record reproduced vs published numbers side by side. The method is compared with the *reproduced* number under one protocol; a gap beyond the paper's reported spread is a finding to explain or disclose (different data vintage, missing tuning, a leakage the original had), never a reason to quote the published number instead.
