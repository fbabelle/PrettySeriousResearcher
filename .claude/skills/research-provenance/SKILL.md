...
## One row, two tables, two numbers

When two tables report the same row from two scripts with different inclusion filters (one counts every admitted unit, the other only units with enough follow-up days), every referee reads the pair as an error; three independent reviewers flagged one such pair as an internal inconsistency although both numbers traced to their artefacts. State each table's inclusion filter in its caption, or reconcile both tables to one population (2026-09-25).
