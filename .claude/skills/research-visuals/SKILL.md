...
- **Phase B — at write-up:** each figure is checked against its caption/claim, for legibility at print size, and for main-text-vs-appendix duplication. **A pandoc pipe table takes its column widths from the dash counts of its separator row** once any row is wider than the text block, so nine equal `---` cells give nine equal columns and a long first column overflows by a few points; size the dashes in proportion to the content, put one library or condition per table and name it in the caption, and shorten cell labels before shrinking the font (earned 2026-09-19). **Draw every figure at the width it is printed.** A 5.6-inch canvas inserted at 40% of a 6.5-inch text width prints its 8 pt labels below 4 pt, and the owner's verdict was "every figure is too small"; set `figsize` to the print width (insertion width 80-97% for a single-column page), keep in-figure type at 8-9 pt, and re-check label collisions after the redraw, since they move. Lines that differ only by dash pattern within one colour blur together: give each arm one colour and split the second factor into panels (earned 2026-09-19).
