...
- **Pass 4 — adversarial final QA** (fresh-eyes agents, after all edits): (a) **semantic** cross-reference check — every §/Table/Figure/Appendix pointer's target must *contain what the citing sentence claims*, not merely exist; (b) stray-content sweep — orphaned "above/below", transitions into deleted text, unbalanced markup, and **placeholders that promise what is not there** (a sentence saying "one table lists every X" with no table, an "extended tables" section naming tables that do not exist): a referee reads them as unfinished work, so build the promised object from the design record or delete the promise (earned 2026-09-15); (c) contradiction sweep across five categories — numbers (prose vs its own tables), claim strength, definitions, verdicts, scope — noting that **compression is the top contradiction source** (a cut qualifier quietly falsifies the sentence that remains); (d) reference truthfulness — cited↔listed both ways, entries spot-verified against live sources.

**Describe what ran, not what the engine can do.** Every mechanism paragraph and every hypothesis of a stated guarantee must match the code path the reported experiments executed (trace the call site: which weights, which universe, which normalisation). A capability the engine has but no reported cell exercised gets one clause marked as unused, never the main construction; otherwise a referee attacks the described construction, the round is wasted, and the correction reads as a retraction (earned 2026-09-16: the described merge was never run; every unit was a singleton).

**Editing safety rails.** Snapshot the draft before the first edit and diff numeric tokens after every pass (a version-consistency guard: a revision may delete numbers, never introduce them; the only allowed additions are new references' bibliographic tokens, whitelisted by eye). Renumbering sections after a merge/delete: retarget references to the deleted section *first*, then ascending replace-all sweeps (`§7.9→§7.8`, then `§7.10→§7.9`, …), then grep-verify zero stale tokens — and re-check any text you pre-wrote in post-renumber terms, which the sweep will re-shift. Better still, never hard-code a section, table or figure number: label every header and float from the first draft and point at it with `\ref`, and have the build assert that no literal section sign or bare "Table N" survives in the source. Hard-coded numbers rot silently the moment a chapter is inserted — every one of a draft's 27 section pointers was wrong after one insertion, and the number-matching passes did not notice because a wrong section number is not a wrong number (earned 2026-09-15). Emphasis economy: bold marks *claims*, not machinery nouns — a skimming reviewer should reconstruct the argument from bold alone; keep run-in labels consistently styled within a sequence. The conclusion *advocates*: thesis first, contributions enumerated, a quotable closing stance — a fact-list conclusion reads like a diary.
...
- **LLM language polish is chunked and mechanically guarded.** Send the draft to the polishing model one section-sized chunk at a time with an instruction block that forbids touching numbers, math, citation keys, cross-references, headers and tables; then verify each returned chunk against invariants — identical multisets of numbers, math spans, citation keys and cross-reference targets, identical header/table/figure lines, no em dashes, no non-ASCII, a length ratio within a stated band — and **reject a failing chunk wholesale** (keep the original, list it for review) rather than hand-fixing it, which reintroduces exactly the drift the invariants exist to catch. Em dashes are a generated-text tell; remove them at the source and let the polish instruction restructure the sentence (earned 2026-09-15).
- **Statistical wording is exact.** A realised proportion is not the expected quantity a guarantee bounds (FDP is not FDR); a count of admissions is not an error rate; an estimate that contamination or extra information could inflate is not an "upper bound" unless an argument makes it one; "significant" names the test, its sidedness, n and the correction family. Say which quantity was measured and let the guarantee's own quantity be named separately (earned 2026-09-16, both shapes introduced by a revision and caught by the verification round).
...
## Prefer the direct observation to the clever reconstruction

An argument built from counterfactual re-runs ("had the pipeline held a random subset of the same size, the metric would have been ...") is expensive to compute, expensive to read, and vulnerable to every objection about how the counterfactual was built. The moment the instrumentation records the quantity directly — a per-unit ledger, an attribution accrued every period — rewrite the section around the direct comparison and keep the counterfactual as a robustness check, not the argument. In this project the question "why does the ungated book outperform?" went from four synthetic books and a three-term decomposition to one table of four rows in which the two pipelines differ on exactly one family; the reader sees the mechanism instead of being walked to it (2026-09-13).
