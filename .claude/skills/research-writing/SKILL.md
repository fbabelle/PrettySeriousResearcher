...
- **Floats discipline**: wrap figures as `[tbp]` floats (not inline images) so LaTeX packs pages; cap widths to `\linewidth`; every float referenced in text (audit — orphan captions are a recurring real defect).
