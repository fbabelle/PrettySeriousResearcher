...
- **Before submission** — a fresh adversarial read of the whole argument. **Audit the audit:** when a self-audit drove the last revision, hand the audit report *and* the revised draft together to a model family that did not write either, act on what it finds, and run one more round; a self-audit shares its author's blind spots and its own report is the cheapest attack surface (owner-required standard step, 2026-09-15). Treat the attacker's report as claims, not verdicts: check every item against the code path that actually ran and against the artefact before editing anything, and keep a per-item disposition (severity, finding on inspection, action) as the record; in one pass a share of the blockers were wrong on the facts (the code applied a rule a day earlier than the appendix said), moot (a mechanism the paper described but no reported cell exercised) or built on stale prose. Then run a verification round on the revised draft that asks only resolved / partial / unresolved per item plus any error the revision itself introduced, because a revision written under pressure adds its own (two in one pass, earned 2026-09-16).
...
**Verify every citation a panel member produces before it enters the verdict.** Cross-family panel members hallucinate references with confident detail (earned 2026-09-07: of three papers cited as "external facts", one existed, one did not, one was a different paper by the same authors). Run the `research-references` check on each; keep the *point* if it stands without the citation, and record verified / not-found per item in the verdict.
