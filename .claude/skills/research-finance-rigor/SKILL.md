...
- **A diversified book's Sharpe rewards breadth, not certification.** An equal-notional sum of many weak sleeves has a higher after-cost Sharpe than a certified few, because variance falls with the number of sleeves and a factor that misses the certification threshold usually still has a small positive edge. So a gate that admits everything can beat a strict one on portfolio Sharpe while admitting far more false factors — seen twice, under a composite book and again under sleeves with a turnover cap and a beta hedge (2026-09-09). Never let a portfolio Sharpe stand in for selection quality: compare gates at equal breadth (same number of sleeves, or top-N by the gate's own evidence), report the per-sleeve Sharpe of certified vs uncertified factors, and state which of the two the paper's claim is about.
