...
- **No leakage / correct splits** — for temporal or market targets, split by **time** and use walk-forward/out-of-sample evaluation; for non-temporal finance QA or document tasks, use entity/document/group-disjoint splits when repeated entities could leak. Random row splits are acceptable only when independence is defensible. **The paper carries a fixed-before-data ledger:** one paragraph in the design section that sorts every input into fixed in advance and never tuned / walk-forward by construction / in sample (a market constant, a construction chosen after a first pass, overlapping start years), then says whether there is a held-out period beyond the walk-forward and how many markets; an owner's "is this all in sample?" is answered by that ledger, not by a limitations sentence (earned 2026-09-18). **When a guarantee's null and the paper's error label differ, decompose the label:** if the referee tests "no edge" but a "false admission" means "edge below the viability threshold", report how many false admissions have a negative edge, a positive edge below the threshold, and how many the retirement rule had already removed; the composition, not the count, is what a referee asks for, and it is a zero-cost ledger pass (the frozen referee admitted no negative-edge factor in 200 cells; earned 2026-09-18).
- **Multiple-testing and selection correction** — define the family and estimand first, then choose the appropriate control. Deflated Sharpe/PBO address strategy-selection overfit; White's Reality Check or Hansen's SPA compare data-mined rules; FWER/FDR procedures address hypothesis families. They are not interchangeable. Report all relevant trials and never present best-of-N unadjusted. **Trials include what an agent loop tried:** every idea an automated loop screened, engineered, evolved or refined (including changes proposed by an ablation, a reviewer or a reflection) is a trial in the selection family, and the slice it was screened on is in-sample. An autonomous idea-search loop is a strategy-selection process, so its full candidate count enters DSR/PBO, not only the variants that reached the paper.
