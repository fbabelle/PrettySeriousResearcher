...
- **A human expert's review is triaged claim by claim against the code before any of it is accepted.** A strong outside review mixes three kinds of statement: analyses it asks for (do them), predictions (test them), and diagnoses of your own system (check them, because the reviewer has not read the code). In one review the central demands were right and two confident diagnoses were wrong: a yield inversion attributed to the retirement detector was a crowding mechanism built into the synthetic world, and a "pure re-computation" was only half one because the quantity fed back into the controller. Write a disposition table with a row per point: what was found, what changed, what was not done and why. Record the points where the reviewer was wrong as carefully as the ones conceded; both are what the next referee will ask (earned 2026-09-19).
