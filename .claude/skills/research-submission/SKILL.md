...
Define the first preprint as **the current draft plus three cheap passes and nothing new**: the mock-review packet with only the top hard defects fixed, the title reconciled with the paper's spine, and the language pass; every remaining free analysis that fits goes in, every new experiment (a second market, a paid re-run, a heavier robustness study) is v2 material. Name the cut point to the owner as a list of the passes still owed, with the one decision (the title) they must make, and check the target venue's preprint policy once before posting (earned 2026-09-18). **What stays out of the preprint is the owner's call, and that includes the AI-involvement statement:** when the owner drops the reproducibility appendix (no artefact address yet), offer the disclosure once as one unnumbered paragraph before the references, saying plainly that recordings and code are not part of the preprint; if the owner rules it out too, remove it, record in the handoff and the review packet that both return at journal submission, and check the venue's disclosure policy at that point rather than now (earned 2026-09-19; the first form of this rule made the disclosure mandatory and the owner overruled it one round later).
...
- **Scan the exact upload for machine-writing tells** (`research-writing/scripts/ai_style_scan.py --strict` on the manuscript, supplement, highlights and cover letter): em-dash and semicolon density, arrows/ellipses in prose, signature vocabulary. A generated-looking first page is a silent desk-reject reason that no letter will name; the removal pass lives in `research-writing` (`references/ai-writing-tells.md`).
...
- **Artifact / code release:** double-blind review → an anonymized mirror (e.g. an anonymous-repo service) with an install + one-command-reproduces-the-headline-table README; acceptance → public repo + archival DOI (e.g. Zenodo). License stated; data availability consistent with the licenses recorded at experiment time (`docs/local-env.md`; finance data per `research-finance-rigor`). The manuscript's own **data-and-code availability statement** is written to what can actually be released, not to the ideal: name the vendor-licensed inputs that may not be redistributed and the route by which the code is obtained (public repo, or on request to the corresponding author — mark one if the statement points at one). An over-promising statement is enforced at acceptance, when withdrawing it is expensive (earned 2026-09-19).
- **Stage the portal fields** (OpenReview / CMT / EasyChair / Editorial Manager): draft every form field — title, abstract (portal field limits differ from the paper's abstract), keywords, COI/conflicts — into `docs/submission/<venue>-form.md` for the user to paste and confirm.
...
- **Freeze the submitted artifact** (copy the exact uploaded PDF/zip under a `pending_approval/` or `submitted/` folder) so later trims and polish on the living draft never blur what the venue actually saw.
...
- **Reviewed reject** → fold the reviews into the draft **first** (real reviews are the highest-value input the paper will ever get — log each point into the claims ledger / weakness list so the next submission is stronger), then route down the `research-venue-selection` fallback chain, re-verifying the next target's deadline. Never resubmit unchanged.
- **Desk reject** (rejected without external review — the letter cites generic grounds such as novelty, significance, language, "likelihood to survive full review") → there are **no reviews to fold**. Do not step to the next rung blind: run the `research-mock-review` **editor pass** on every remaining candidate, fix the first-page signals it names (title/abstract lead, length vs venue median, contribution-type framing, cover-letter credibility), consider a pre-submission enquiry, and only then resubmit. Log the desk reject (date, editor, wording) in the venue plan; decision letters are usually marked confidential — quote them internally, never forward them. A publisher **transfer-desk** offer that arrives with the rejection is a lead, not an endorsement (`research-venue-selection`).
