...
0. **Build the limitation inventory of the named SOTA before ideating.** Extract its limitations from the paper, its code, its OpenReview reviews and any reproduction reports, one row per limitation with the evidence. Then a second pass — another model family when one is available — asks only "is this list sufficient to guide an improvement, and which actionable limitation is missing?", and the extractor expands the list until that reader finds nothing new or a stated round cap (2–3) is hit. Candidates are generated *against inventory rows*, which is what makes step 2's "limitation it removes" field honest rather than retrofitted (the Limitation Extractor/Verifier loop of ScientistTwo, arXiv:2609.19644 §3.1).
...
4. **De-dup for diversity.** Ensure the shortlist spans **distinct clusters**, not near-duplicates of one idea. Keep the cards that did not make the slate as a **reserve pool**, ordered by novelty, with the limitation row each one targets: when a loop-back (failed solvability gate, failed screen, reflection) needs new candidates, draw the next untried reserve cards alongside any refined ones instead of only mutating the idea that just failed, which keeps the search from circling one early idea.
